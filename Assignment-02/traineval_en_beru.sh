...
BINARIZED_DATA=data/ted_binarized/beru_spm8000/O2M/
MODEL_DIR=checkpoints/ted_beru_spm8000/O2M/
...
fairseq-generate $BINARIZED_DATA \
	...
	--lang-pairs en-be,en-ru \
 	--encoder-langtok tgt \
    --source-lang en --target-lang be \
    --path $MODEL_DIR/checkpoint_best.pt \
    --batch-size 32 \
    --remove-bpe sentencepiece \
    --beam 5  | grep ^H | cut -c 3- | sort -n | cut -f3- > "$MODEL_DIR"/test_b5.pred

python score.py "$MODEL_DIR"/test_b5.pred "$RAW_DATA"/test.be \
...
python score.py "$MODEL_DIR"/valid_b5.pred "$RAW_DATA"/dev.be \
    --src "$RAW_DATA"/dev.en \
	--comet-dir $COMET_DIR \
